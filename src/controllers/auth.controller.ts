...
export const forgotPassword = async (
  ...
  if (response.done) {
    res
      ...
      .send({ ...response, message: "Mail with reset token sent." });
  } else {
    res.status(httpStatus.EXPECTATION_FAILED).send(response);
  }
};
...
export const resetPassword = async (
  ...
  if (response.done) {
    res
      .status(httpStatus.CREATED)
      .send({ ...response, message: "Password reset." });
  } else {
    res.status(httpStatus.EXPECTATION_FAILED).send(response);
  }
};
...
export const changePasswordOrEmail = async (
  req: express.Request,
  res: express.Response,
) => {
  ...
  if (!response.done) {
    ...
    res
      .status(httpStatus.CREATED)
      .send({ ...response, message: "Credentials changed" });
  }
};
