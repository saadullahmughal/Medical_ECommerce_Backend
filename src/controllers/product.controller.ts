...
export const addProduct = async (
  ...
  if (response.done) {
    res
      ...
      .send({ ...response, message: "Product added" });
  } else {
...
export const updateProduct = async (
  ...
  const response = await updateProductData(productTitle, newProductDetails);
  if (response.done) {
    res
      .status(httpStatus.CREATED)
      .send({ ...response, message: "Updated successfully" });
  } else {
    res.status(httpStatus.EXPECTATION_FAILED).send(response);
  }
};
...
export const addProductReview = async (
  ...
  if (response.done) {
    res
      .status(httpStatus.CREATED)
      .send({ ...response, message: "Review Posted" });
  } else {
    res.status(httpStatus.EXPECTATION_FAILED).send(response);
  }
};
