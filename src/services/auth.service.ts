...
const accessTokenExpiry = ms(
  process.env?.ACCESS_EXPIRY
    ? typeof ms(process.env?.ACCESS_EXPIRY as StringValue) === "number"
      ? (process.env?.ACCESS_EXPIRY as StringValue)
      : "1 hr"
    : "1 hr",
);
...
export const signUpService = async (reqBody: Record<string, unknown>) => {
  try {
    const record = {
      ...reqBody,
      password: bcrypt.hashSync(reqBody?.password as string, salt),
    };
...
export const logInService = async (reqBody: Record<string, unknown>) => {
  try {
    const record = {
      password: bcrypt.hashSync(reqBody?.password as string, salt),
    ...
    const refreshToken = genToken({ uid: tokenPayload?.userName }, "300d");
    await RefreshTokens.create({ token: refreshToken });
    const token = genToken(tokenPayload, accessTokenExpiry)?.toString();
    return {
      done: true,
...
export const logOutService = async (token: string) => {
  try {
    ...
    if (deleteResults.deletedCount != 0) return { done: true };
...
export const forgotPasswordService = async (
  ...
  try {
    ...
    if (!user) return { done: false, message: "No such user exists" };
      ...
      await sendResetLink(user?.email, mongoResult.token);
      return { done: true };
    }
  } catch (error) {
    return { done: false, message: parseMongoError(error) };
...
export const resetPasswordService = async (
  ...
  try {
    ...
    console.log(token);
    const findResults = await resetToken.exists({
      token: BigInt(token as string),
      userID: userName,
    });
    console.log(findResults);
    if (!findResults) return { done: false, message: "Invalid token" };
    console.log("Found");
    const passHash = genHash(password as string);
...
export const refreshTokenService = async (token: string) => {
  try {
    ...
    const userName = tokenPayload?.uid;
    ...
    if (userFound) {
      const newTokenPayload = { ...userFound?.toObject() };
      ...
      const newAccessToken = genToken(newTokenPayload, accessTokenExpiry);
...
export const changePassword = async (
  ...
  try {
    const updated = await User.updateOne(
      { userName: userName, password: genHash(oldPassword) },
...
export const alterEmail = async (oldEmail: string, newEmail: string) => {
  ...
  try {
    ...
    updated = await FormData.updateOne(
      { email: oldEmail },
      { email: newEmail },
      { session },
    ).exec();
