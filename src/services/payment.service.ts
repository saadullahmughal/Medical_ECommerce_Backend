...
export const addToCart = async (
  itemData: { item: string; count: number; cartID?: string },
  userName: string,
) => {
  try {
    let overflow = false;
    ...
    if (!itemData.cartID) {
      ...
      if (matchedItemsInCart.length == 0) {
        ...
        await Order.updateOne(
          {
            _id: new ObjectId(itemData.cartID),
            "orderItems.productTitle": itemData.item,
          },
          { $inc: { "orderItems.$.productCount": itemData.count } },
        );
      }
      const result = await Order.findByIdAndUpdate(itemData.cartID, {
        $inc: {
      ...
      await stripe.paymentIntents.update(fetchedData.transactionID, {
        amount: result?.netTotal * 100,
      });
      return {
        done: true,
...
export const getCart = async (cartID: string, userName: string) => {
  try {
    ...
    for (const index in cart.orderItems) {
      ...
      const fetchedData = await Product.findOne({
...
export const placeOrder = async (
  ...
  try {
    ...
    {
      const orderItems = orderData?.orderItems as [Record<string, unknown>];
      let sum = 0;
      for (const item of orderItems) {
        sum += (item?.productCount as number) * (item?.unitCost as number);
        const result = await Product.updateOne(
          { title: item.productTitle, quantity: { $gte: item.productCount } },
          { $inc: { quantity: -(item.productCount as number) } },
          { session },
        );
    ...
    return {
      ...
      message: (error as Stripe.errors.StripeError).message,
    };
  }
};
...
export const getIntentClientSecret = async (
  ...
  try {
    const cart = await Order.findOne(
      { _id: new ObjectId(cartID), userName: userName },
      { transactionID: true },
    );
    if (!cart) throw new Error("Invalid cart ID");
  ...
  try {
    ...
    for (const item of result.orderItems || []) {
      const updateResult = await Product.updateOne(
        { title: item.productTitle, quantity: { $gte: item.productCount } },
        { $inc: { quantity: -item.productCount } },
      );
